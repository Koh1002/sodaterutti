  ...
  return (
    <header className="relative z-40">
      <div className="max-w-lg mx-auto px-4 py-2.5 flex items-center justify-between">
        {/* 左: 時計 */}
        <div className="bg-black/20 backdrop-blur-md rounded-full px-3.5 py-1.5">
          <span className="text-white text-sm font-bold tabular-nums">{timeString}</span>
        </div>

        {/* 中央: 世代・日数 */}
        {character && (
          <div className="flex items-center gap-2">
            <span className="bg-white/25 backdrop-blur-md text-white text-sm font-bold px-3 py-1 rounded-full">
        ...
        <button
          ...
          className="relative bg-black/20 backdrop-blur-md rounded-full w-10 h-10 flex items-center justify-center text-white hover:bg-black/30 transition active:scale-95"
        >
        ...
        {menuOpen && (
          <>
            ...
            <motion.div
              ...
              className="absolute right-4 top-full mt-1 z-50 w-52 bg-white/95 backdrop-blur-xl rounded-2xl shadow-xl border border-gray-200/50 overflow-hidden"
            >
              {menuItems.map((item, i) => (
                <button
                  key={item.label}
                  onClick={item.onClick}
                  className={`w-full flex items-center gap-3 px-4 py-3.5 text-left hover:bg-gray-100/80 active:bg-gray-200/80 transition text-sm ${
